...
sp_mean <- specific[,4:12] %>% sapply(mean, na.rm=TRUE)
sp_med <- specific[,4:12] %>% sapply(median, na.rm=TRUE)
sp_count <- specific[,4:12] %>% sapply(function(x) sum(!is.na(x)))
sp_min <- specific[,4:12] %>% sapply(min, na.rm=TRUE)
sp_max <- specific[,4:12] %>% sapply(max, na.rm=TRUE)
sp_25p <- specific[,4:12] %>% sapply(function(x) quantile(x,probs=0.25,na.rm = TRUE))
sp_75p <- specific[,4:12] %>% sapply(function(x) quantile(x,probs=0.75,na.rm = TRUE))
...
```{r echo=FALSE}
av_mean <- ad_valorem[,4:12] %>% sapply(mean, na.rm=TRUE)
av_med <- ad_valorem[,4:12] %>% sapply(median, na.rm=TRUE)
av_count <- ad_valorem[,4:12] %>% sapply(function(x) sum(!is.na(x)))
av_min <- ad_valorem[,4:12] %>% sapply(min, na.rm=TRUE)
av_max <- ad_valorem[,4:12] %>% sapply(max, na.rm=TRUE)
av_25p <- ad_valorem[,4:12] %>% sapply(function(x) quantile(x,probs=0.25,na.rm = TRUE))
av_75p <- ad_valorem[,4:12] %>% sapply(function(x) quantile(x,probs=0.75,na.rm = TRUE))
