...
18. Are current Column 2 tariffs in 1962 Smoot Hawley or the 1946 tariffs?
...
10. ~~Add Schedule A tariff data from 1946 (last available before Geneva 1947)~~
...
```{r}
c <- shortnames %>%
  filter( is.na(Sp_SH) != is.na(Un_SH) & is.na(Sp_A) != is.na(Un_A) & is.na(Sp_Ge) != is.na(Un_Ge) &
...
kbl(sum_specific_pc,digits=2,booktabs=T) %>%
  kable_styling(position = "center", latex_options = "hold_position")
```
...
kbl(sum_specific_bySched,digits=2,booktabs=T) %>%
  kable_styling(position = "center", latex_options = "hold_position")
```
...
sum_av_nI_pc <- subset(ad_valorem, is.na(Interval)) %>%   
  ...
  summarize(SH_mean = mean(Ad_Valorem_SH, na.rm=TRUE),
            ...
            n = n()) %>%
  ungroup() # ungrouping variable is a good habit to prevent errors
kbl(sum_av_nI_pc,digits=2,booktabs=T) %>%
  kable_styling(position = "center", latex_options = "hold_position")
```
...
kbl(sum_av_bySched_nI,digits=2,booktabs=T) %>%
  kable_styling(position = "center", latex_options = "hold_position")
```
...
```{r echo=TRUE}
# we removed the "before" variables once we verified that they were exactly the same as Smoot Hawley
